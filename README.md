...
We must define our own validation vocabulary, based on the Vecorel Schema [data types](datatypes.md).
Nevertheless, Vecorel Schema can be translated into JSON Schema.
...
- `items` (required, object only, sub-schema must be compliant to Vecorel Schema)
- `minItems`
- `maxItems`
- `uniqueItems`
- `contains` (sub-schema must be compliant to Vecorel Schema)

### Object data type

- `required`
- `properties` (sub-schemas must be compliant to Vecorel Schema)
- `additionalProperties`
  ...
  Sub-schemas must be compliant to Vecorel Schema)
- `patternProperties` (sub-schemas must be compliant to Vecorel Schema)
- `minProperties`
- `maxProperties` (>= 1)
